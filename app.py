...
st.markdown(
    ...
    <style>
        ...
        /* Everything below uses Streamlit's theme CSS variables (var(--text-color),
           var(--background-color), var(--secondary-background-color)) rather than
           fixed hex values, so it automatically follows the viewer's light/dark
           toggle — see config.toml [theme.light] / [theme.dark] for the two
           palettes those variables resolve to. Only the case-file accent colors
           (crimson/sage/amber) stay fixed, since they're mid-tone enough to read
           on both a light and dark surface. */
        h1 {{
        ...
        h2 {{ font-family: 'Source Serif 4', Georgia, serif !important; color: var(--text-color) !important; font-weight: 700 !important; }}
        h3 {{
        ...
        div[data-testid="stMetricLabel"] {{ color: var(--text-color) !important; opacity: 0.65; font-family: 'Inter', sans-serif; }}
        div[data-testid="stMetricValue"] {{ font-family: 'JetBrains Mono', monospace; }}

        .subtle {{ color: var(--text-color); opacity: 0.65; font-size: 0.9rem; }}
        ...
        .sev-CRITICAL {{ background: color-mix(in srgb, {CRIMSON} 15%, var(--background-color)); color: {CRIMSON}; }}
        .sev-HIGH {{ background: color-mix(in srgb, {AMBER} 15%, var(--background-color)); color: {AMBER}; }}
        .sev-MEDIUM {{ background: color-mix(in srgb, {SAGE} 15%, var(--background-color)); color: {SAGE}; }}
        .sev-LOW {{ background: var(--secondary-background-color); color: var(--text-color); opacity: 0.75; }}
...
def classification_banner(level: str):
    ...
    s = TLP[level]
    bg = f"color-mix(in srgb, {s['color']} 15%, var(--background-color))"
    st.markdown(
