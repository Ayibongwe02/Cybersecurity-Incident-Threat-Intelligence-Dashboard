...
TLP = {
    "RED":   {"color": CRIMSON, "bg": "#FBEAE9", "label": "TLP:RED",   "desc": "Not for disclosure — restricted to named recipients"},
    "AMBER": {"color": AMBER,   "bg": "#FCF1EC", "label": "TLP:AMBER", "desc": "Limited disclosure — internal use only"},
    "GREEN": {"color": SAGE,    "bg": "#F1F4EE", "label": "TLP:GREEN", "desc": "Community disclosure — sector peers"},
}
...
st.markdown(
    ...
    <style>
        ...
        h2 {{ font-family: 'Source Serif 4', Georgia, serif !important; color: {CHARCOAL} !important; font-weight: 700 !important; }}
        h3 {{
        ...
        div[data-testid="stMetric"] {{
            background: #FFFFFF; border: 1px solid {HAIRLINE}; border-radius: 8px; padding: 14px 18px;
        }}
        div[data-testid="stMetricLabel"] {{ color: #6B6558 !important; font-family: 'Inter', sans-serif; }}
        div[data-testid="stMetricValue"] {{ font-family: 'JetBrains Mono', monospace; }}

        section[data-testid="stSidebar"] {{ background-color: #FFFFFF; border-right: 1px solid {HAIRLINE}; }}

        .subtle {{ color: #8A8474; font-size: 0.9rem; }}
        ...
        .sev-CRITICAL {{ background: #FBEAE9; color: {CRIMSON}; }}
        .sev-HIGH {{ background: #FCF1EC; color: {AMBER}; }}
        .sev-MEDIUM {{ background: #F1F4EE; color: {SAGE}; }}
        .sev-LOW {{ background: #F4F2EC; color: #8A8474; }}
...
def classification_banner(level: str):
    ...
    s = TLP[level]
    st.markdown(
        f'<div class="tlp-banner" style="background:{s["bg"]};color:{s["color"]};'
